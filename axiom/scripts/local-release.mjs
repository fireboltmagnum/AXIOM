...
const packages = [
	{ directory: "packages/ai", name: "@axiom/ai" },
	{ directory: "packages/tui", name: "@axiom/tui" },
	{ directory: "packages/agent", name: "@axiom/agent-core" },
	{ directory: "packages/coding-agent", name: "@axiom/coding-agent" },
];
...
function prepareOutputDirectory(options, repoRoot) {
	if (!options.outDir) {
		return mkdtempSync(join(tmpdir(), "axiom-local-release-"));
	}
...
function buildBunBinaryRelease(targetDirectory, archiveDirectory) {
	...
	const executableName = platform.startsWith("windows-") ? "axiom.exe" : "axiom";
	...
	if (platform.startsWith("windows-")) {
		run("powershell", ["-NoProfile", "-Command", `Compress-Archive -Path '${join(targetDirectory, "*").replaceAll("'", "''")}' -DestinationPath '${join(archiveDirectory, `axiom-${platform}.zip`).replaceAll("'", "''")}' -Force`]);
	} else {
		run("tar", ["-czf", join(archiveDirectory, `axiom-${platform}.tar.gz`), "-C", targetDirectory, "."]);
	}
	return platform;
}

function createAxiomShim(installDirectory) {
	const binDirectory = join(installDirectory, "node_modules", ".bin");
	if (process.platform === "win32") {
		if (existsSync(join(binDirectory, "axiom.cmd"))) {
			writeFileSync(join(installDirectory, "axiom.cmd"), '@ECHO off\r\n"%~dp0node_modules\\.bin\\axiom.cmd" %*\r\n');
			writeFileSync(join(installDirectory, "axiom.ps1"), '& "$PSScriptRoot/node_modules/.bin/axiom.ps1" @args\n');
			return;
		}
		writeFileSync(join(installDirectory, "axiom.cmd"), '@ECHO off\r\n"%~dp0node_modules\\.bin\\axiom.exe" %*\r\n');
		writeFileSync(join(installDirectory, "axiom.ps1"), '& "$PSScriptRoot/node_modules/.bin/axiom.exe" @args\n');
		return;
	}
	symlinkSync(join("node_modules", ".bin", "axiom"), join(installDirectory, "axiom"));
}
...
if (rootPackageJson.name !== "axiom-monorepo") {
...
if (!options.skipInstall) {
	...
	createAxiomShim(nodeInstallDirectory);
	...
	if (!options.skipBunInstall) {
		...
		createAxiomShim(bunInstallDirectory);
	}
}
...
if (!options.skipInstall) {
	...
	console.log(`  ${join(outDir, `axiom-${binaryPlatform}.${String(binaryPlatform).startsWith("windows-") ? "zip" : "tar.gz"}`)}`);
	console.log("\nRun the local Bun binary release from outside the repository:");
	console.log(`  ${join(binaryDirectory, String(binaryPlatform).startsWith("windows-") ? "axiom.exe" : "axiom")} --help`);

	console.log("\nIsolated npm install:");
	console.log(`  ${nodeInstallDirectory}`);
	console.log("\nRun the locally packed npm CLI from outside the repository:");
	console.log(`  ${join(nodeInstallDirectory, process.platform === "win32" ? "axiom.cmd" : "axiom")} --help`);
	...
	if (!options.skipBunInstall) {
		...
		console.log(`  ${join(bunInstallDirectory, process.platform === "win32" ? "axiom.cmd" : "axiom")} --help`);
	}
}
