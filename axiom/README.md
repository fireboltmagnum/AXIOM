...
>
> v2 base. Forked from [Pi](https://github.com/earendil-works/pi) (MIT, by Mario Zechner / earendil-works). AXIOM-specific cognitive layers are not in this base yet — see `AXIOM research paper full.txt` for the target architecture.

## Packages

| Package | Description |
|---------|-------------|
| **[@axiom/ai](packages/ai)** | Unified multi-provider LLM API (OpenAI, Anthropic, Google, Bedrock, Mistral, etc.) |
| **[@axiom/agent-core](packages/agent)** | Agent runtime with tool calling, transport, sessions, compaction |
| **[@axiom/coding-agent](packages/coding-agent)** | Interactive coding agent CLI (`AXIOM`/`axiom`) with bash, edit, write, grep, find, read tools |
| **[@axiom/tui](packages/tui)** | Terminal UI library with differential rendering |
