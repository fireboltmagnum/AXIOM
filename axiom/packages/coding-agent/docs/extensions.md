...
> **Placement for /reload:** Put extensions in `~/.axiom/agent/extensions/` (global) or `.axiom/extensions/` (project-local) for auto-discovery. Use `pi -e ./path.ts` only for quick tests. Extensions in auto-discovered locations can be hot-reloaded with `/reload`.
...
```typescript
import type { ExtensionAPI } from "@axiom/coding-agent";
...
| `.axiom/extensions/*.ts` | Project-local |
| `.axiom/extensions/*/index.ts` | Project-local (subdirectory) |
...
| Package | Purpose |
|---------|---------|
| `@axiom/coding-agent` | Extension types (`ExtensionAPI`, `ExtensionContext`, events) |
| `typebox` | Schema definitions for tool parameters |
| `@axiom/ai` | AI utilities (`StringEnum` for Google-compatible enums) |
| `@axiom/tui` | TUI components for custom rendering |
...
```typescript
import { isToolCallEventType } from "@axiom/coding-agent";
...
```typescript
import { isBashToolResult } from "@axiom/coding-agent";
...
```typescript
import { createLocalBashOperations } from "@axiom/coding-agent";
...
```typescript
import { SessionManager } from "@axiom/coding-agent";
...
```typescript
import { Type } from "typebox";
import { StringEnum } from "@axiom/ai";
...
```typescript
import type { AutocompleteItem } from "@axiom/tui";
...
```typescript
import { withFileMutationQueue } from "@axiom/coding-agent";
...
**Important:** Use `StringEnum` from `@axiom/ai` for string enums. `Type.Union`/`Type.Literal` doesn't work with Google's API.
...
```typescript
import { createReadTool, createBashTool, type ReadOperations } from "@axiom/coding-agent";
...
```typescript
import { createBashTool } from "@axiom/coding-agent";
...
```typescript
import { keyHint } from "@axiom/coding-agent";
...
```typescript
import { Text, Component } from "@axiom/tui";
...
```typescript
import { CustomEditor, type ExtensionAPI } from "@axiom/coding-agent";
import { matchesKey } from "@axiom/tui";
...
```typescript
import { highlightCode, getLanguageFromPath } from "@axiom/coding-agent";
