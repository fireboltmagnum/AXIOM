...
**Source:** [`@axiom/tui`](https://github.com/earendil-works/pi-mono/tree/main/packages/tui)
...
Import from `@axiom/tui`:

```typescript
import { Text, Box, Container, Spacer, Markdown } from "@axiom/tui";
```
...
```typescript
import { getMarkdownTheme } from "@axiom/coding-agent";
import { Markdown } from "@axiom/tui";
...
For letting users pick from a list of options. Use `SelectList` from `@axiom/tui` with `DynamicBorder` for framing.

```typescript
import type { ExtensionAPI } from "@axiom/coding-agent";
import { DynamicBorder } from "@axiom/coding-agent";
import { Container, type SelectItem, SelectList, Text } from "@axiom/tui";
...
```typescript
import { BorderedLoader } from "@axiom/coding-agent";
...
For toggling multiple settings. Use `SettingsList` from `@axiom/tui` with `getSettingsListTheme()`.

```typescript
import { getSettingsListTheme } from "@axiom/coding-agent";
import { Container, type SettingItem, SettingsList, Text } from "@axiom/tui";
...
import { matchesKey, truncateToWidth } from "@axiom/tui";
