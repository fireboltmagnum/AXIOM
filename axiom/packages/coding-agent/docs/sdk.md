...
```bash
npm install @axiom/coding-agent
```
...
```typescript
import { createAgentSession, SessionManager } from "@axiom/coding-agent";
...
The `Agent` class (from `@axiom/agent-core`) handles the core LLM interaction. Access it via `session.agent`.
...
- Project extensions (`.axiom/extensions/`)
- Project skills:
  - `.axiom/skills/`
  - `.agents/skills/` in `cwd` and ancestor directories (up to git repo root, or filesystem root when not in a repo)
- Project prompts (`.axiom/prompts/`)
...
import { AuthStorage, ModelRegistry } from "@axiom/coding-agent";
...
```typescript
import { createAgentSession, DefaultResourceLoader } from "@axiom/coding-agent";
...
```typescript
import { createAgentSession } from "@axiom/coding-agent";
...
import { createAgentSession, defineTool } from "@axiom/coding-agent";
...
Extensions are loaded by the `ResourceLoader`. `DefaultResourceLoader` discovers extensions from `~/.axiom/agent/extensions/`, `.axiom/extensions/`, and settings.json extension sources.
...
```typescript
import { createEventBus, DefaultResourceLoader } from "@axiom/coding-agent";
...
```typescript
import { createAgentSession, SettingsManager, SessionManager } from "@axiom/coding-agent";
...
2. Project: `<cwd>/.axiom/settings.json`
