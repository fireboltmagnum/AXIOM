...
By default, `install` and `remove` write to user settings (`~/.axiom/agent/settings.json`). Use `-l` to write to project settings (`.axiom/settings.json`) instead. Project settings can be shared with your team, and pi installs any missing packages automatically on startup.
...
- User installs go under `~/.axiom/agent/npm/`.
- Project installs go under `.axiom/npm/`.
...
- Cloned to `~/.axiom/agent/git/<host>/<path>` (global) or `.axiom/git/<host>/<path>` (project).
...
AXIOM does not host a hosted package gallery. Packages are discovered via npm search (`npm search axiom-package`) and installed directly. The `video` / `image` fields below are kept for tooling compatibility — they have no current viewer.
...
Pi bundles core packages for extensions and skills. If you import any of these, list them in `peerDependencies` with a `"*"` range and do not bundle them: `@axiom/ai`, `@axiom/agent-core`, `@axiom/coding-agent`, `@axiom/tui`, `typebox`.
...
Use `pi config` to enable or disable extensions, skills, prompt templates, and themes from installed packages and local directories. Works for both global (`~/.axiom/agent`) and project (`.axiom/`) scopes.
