...
import { type ImageContent, modelsAreEqual } from "@axiom/ai";
import { ProcessTerminal, setKeybindings, TUI } from "@axiom/tui";
...
export async function main(args: string[], options?: MainOptions) {
	resetTimings();
	const offlineMode = args.includes("--offline") || isTruthyEnvFlag(process.env.AXIOM_OFFLINE);
	if (offlineMode) {
		process.env.AXIOM_OFFLINE = "1";
		process.env.AXIOM_SKIP_VERSION_CHECK = "1";
	}
	...
	const startupBenchmark = isTruthyEnvFlag(process.env.AXIOM_STARTUP_BENCHMARK);
	if (startupBenchmark && appMode !== "interactive") {
		console.error(chalk.red("Error: AXIOM_STARTUP_BENCHMARK only supports interactive mode"));
		process.exit(1);
	}
