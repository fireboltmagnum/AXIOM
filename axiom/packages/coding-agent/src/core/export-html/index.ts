import type { AgentState } from "@axiom/agent-core";
...
/**
 * Drop AXIOM's internal IP-retry follow-up messages from a session entry list
 * before display/export. These are synthetic user messages that the runtime
 * injects to make the model retry after an integrity-check failure; the human
 * never typed them and should not see them in the rendered transcript.
 */
function stripAxiomInternalEntries(entries: SessionEntry[]): SessionEntry[] {
...
export async function exportSessionToHtml(
	...
	const entries = stripAxiomInternalEntries(sm.getEntries());
...
export async function exportFromFile(inputPath: string, options?: ExportOptions | string): Promise<string> {
	...
	const sessionData: SessionData = {
		header: sm.getHeader(),
		entries: stripAxiomInternalEntries(sm.getEntries()),
		leafId: sm.getLeafId(),
