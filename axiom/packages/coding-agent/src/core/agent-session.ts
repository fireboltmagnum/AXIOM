...
import type { AxiomEffortLevel, AxiomFeatureKey, ResolvedAxiomSettings, SettingsManager } from "./settings-manager.ts";
...
export type AgentSessionEvent =
	...
	| { type: "axiom_effort_changed"; settings: ResolvedAxiomSettings }
	| {
			type: "compaction_end";
...
export class AgentSession {
	...
	private _fallbackRetryActive = false;
	...
	// AXIOM runtime layer: local routing, trace capture, and deterministic IP validation.
	private _axiomRuntime: AXIOMRuntime;
	private _activeAxiomTraceId: string | undefined;
	private _activeAxiomUserText = "";
	private _activeAxiomClassification: AxiomTaskClassification | undefined;
	/** Current task's plan (Step-Back abstraction, recalls, ASCoT depth). Used for
	 * system-prompt augmentation and failure capture. Cleared at task end. */
	private _activeAxiomPlan: AxiomTaskPlan | undefined;
	private _axiomGraphTracker: GraphExecutionTracker | undefined;
	/** IP-validation retry counter for the current task. Reset on every new task. */
	private _axiomIPRetryCount = 0;
	/** Tool names used in the current task, in the order they were called. Drives
	 * Skill Evolution capture on successful completion. */
	private _axiomToolsUsedThisTask: string[] = [];
	...
	/** True once the current assistant stream has been aborted by streaming
	 * validation, so we don't queue more than one retry per message. */
	private _axiomStreamAbortPending = false;
	private _repairLoop: RepairLoop;
	private _repairLoopAttempts = 0;
	private _repairLoopLastSignature: string | undefined;
	...
	constructor(config: AgentSessionConfig) {
		...
		this._axiomRuntime = new AXIOMRuntime({
			...
			settings: this.settingsManager.getAxiomSettings(),
		});
		this._repairLoop = new RepairLoop({ cwd: this._cwd });
	...
	private _refreshAxiomRuntime(): void {
		this._axiomRuntime = new AXIOMRuntime({
			cwd: this._cwd,
			sessionId: this.sessionManager.getSessionId(),
			settings: this.settingsManager.getAxiomSettings(),
		});
		this._syncAxiomManagedTools();
	}

	private _shouldAutoEnableUnderstandCode(): boolean {
		const settings = this.settingsManager.getAxiomSettings();
		return (
			settings.enabled &&
			settings.codeUnderstanding &&
			(!this._allowedToolNames || this._allowedToolNames.has("understand_code"))
		);
	}

	private _shouldAutoEnableCodeGraph(): boolean {
		const settings = this.settingsManager.getAxiomSettings();
		return (
			settings.enabled && settings.codeGraph && (!this._allowedToolNames || this._allowedToolNames.has("code_graph"))
		);
	}

	private _shouldAutoEnableFlowGraph(): boolean {
		const settings = this.settingsManager.getAxiomSettings();
		return (
			settings.enabled && settings.flowGraph && (!this._allowedToolNames || this._allowedToolNames.has("flow_graph"))
		);
	}

	private _shouldAutoEnableKnowledgeGraph(): boolean {
		const settings = this.settingsManager.getAxiomSettings();
		return (
			settings.enabled &&
			settings.knowledgeGraph &&
			(!this._allowedToolNames || this._allowedToolNames.has("knowledge_graph"))
		);
	}

	private _shouldAutoEnableSparseTreeGrep(): boolean {
		const settings = this.settingsManager.getAxiomSettings();
		return (
			settings.enabled &&
			settings.sparseTreeGrep &&
			(!this._allowedToolNames || this._allowedToolNames.has("sparse_tree_grep"))
		);
	}

	private _shouldAutoEnablePlaywrightCli(): boolean {
		const settings = this.settingsManager.getAxiomSettings();
		return (
			settings.enabled &&
			settings.playwrightCli &&
			(!this._allowedToolNames || this._allowedToolNames.has("playwright_cli"))
		);
	}
	...
	private _withAxiomManagedTools(toolNames: string[]): string[] {
		...
		if (this._shouldAutoEnableUnderstandCode()) {
			...
			next.delete("understand_code");
		}
		if (this._shouldAutoEnableCodeGraph()) {
			next.add("code_graph");
		} else {
			next.delete("code_graph");
		}
		if (this._shouldAutoEnableFlowGraph()) {
			next.add("flow_graph");
		} else {
			next.delete("flow_graph");
		}
		if (this._shouldAutoEnableKnowledgeGraph()) {
			next.add("knowledge_graph");
		} else {
			next.delete("knowledge_graph");
		}
		if (this._shouldAutoEnableSparseTreeGrep()) {
			next.add("sparse_tree_grep");
		} else {
			next.delete("sparse_tree_grep");
		}
		if (this._shouldAutoEnablePlaywrightCli()) {
	...
	private _syncAxiomManagedTools(): void {
		this.setActiveToolsByName(this._withAxiomManagedTools(this.getActiveToolNames()));
	}
	...
	private _extractMutatedFile(toolName: string, args: unknown): string | undefined {
		...
		const rawPath =
			(args as { path?: unknown; file_path?: unknown }).path ?? (args as { file_path?: unknown }).file_path;
		if (typeof rawPath !== "string" || rawPath.trim().length === 0) return undefined;
		return resolvePath(rawPath, this._cwd, {
			trim: true,
			stripAtPrefix: true,
	...
	private async _maybeRunRepairLoopAtTurnEnd(traceId: string | undefined): Promise<void> {
		...
		this._repairLoopRepeatCount =
			this._repairLoopLastSignature === result.signature ? this._repairLoopRepeatCount + 1 : 1;
		const grewBadly =
			this._repairLoopLastIssueCount > 0 &&
			result.issues.length > Math.max(this._repairLoopLastIssueCount + 4, this._repairLoopLastIssueCount * 2);
	...
	private _recordAxiomGraphToolStart(toolCallId: string, toolName: string): void {
		...
		const update = tracker.onToolStart(toolCallId, toolName);
		if (update) {
			this._axiomRuntime.recordGraphNodeUpdate(this._activeAxiomTraceId, update, tracker.snapshot());
		}
	}
	...
	/**
	 * Spin up a fresh StreamingIPValidator for the upcoming assistant message,
	 * provided ipStreaming is enabled and this task still has abort budget left.
	...
	private _handleStreamChunkFailure(
		...
		this._axiomStreamAbortPending = true;
		this._axiomStreamAbortsThisTask++;
		this._axiomRuntime.recordStreamAbort(traceId, result.resolvedLanguage, this._axiomStreamAbortsThisTask, {
			line: result.line,
			column: result.column,
		});

		// Stop the validator so further chunks don't pile on. Then abort the
		// current stream and queue the hidden retry message. Both ops are
		// fire-and-forget; the agent loop picks the follow-up up on next tick.
		this._axiomStreamValidator?.stop();
		try {
			this.agent.abort();
		} catch {
			// abort may throw if the agent isn't streaming anymore — harmless.
		}
		const feedback = this._axiomRuntime.buildStreamAbortFeedback(result);
		void this.sendUserMessage(feedback, { deliverAs: "followUp" }).catch(() => {
			// Same swallowing pattern as the post-message IP retry path.
		});
	}
	...
	private _handleAgentEvent = async (event: AgentEvent): Promise<void> => {
		...
		if (this._shouldSuppressRetryErrorEvent(event)) {
	...
	private _willRetryAfterAgentEnd(event: Extract<AgentEvent, { type: "agent_end" }>): boolean {
		...
		for (let i = event.messages.length - 1; i >= 0; i--) {
			...
			if (message.role === "assistant") {
				return this._canRetryOrFallback(message as AssistantMessage);
			}
		}
		return false;
	}
	...
	private _recordAxiomEvent(event: AgentEvent): void {
		...
		if (event.type === "message_end" && event.message.role === "assistant" && this._activeAxiomClassification) {
			// Accumulate tool usage so SES can capture the full sequence at task end.
			const assistantMsg = event.message as AssistantMessage;
			for (const part of assistantMsg.content) {
				if (part.type === "toolCall") {
					this._axiomToolsUsedThisTask.push(part.name);
				}
			}

			const validation = this._axiomRuntime.validateAssistantMessage({
				userText: this._activeAxiomUserText,
				classification: this._activeAxiomClassification,
				message: assistantMsg,
			});
			this._axiomRuntime.recordMessageEnd(traceId, assistantMsg, validation);
			...
			if (
				...
				if (!lastAttempt) {
					...
					this._axiomRuntime.recordIPRetry(
						...
						maxRetries,
						errorIssues,
						validation.agentFeedback.length,
					);
					// Fire-and-forget: sendUserMessage queues a followUp turn after the current
					// one finishes. We deliberately do not await; the agent loop will pick it up.
					void this.sendUserMessage(validation.agentFeedback, { deliverAs: "followUp" }).catch(() => {
						// Swallow errors here; failure to enqueue retry is non-fatal and
						// would be hidden by the outer agent loop's error handling anyway.
					});
				}

				// Reflexion: capture the rejection as a persistent lesson on the FINAL
				// attempt (or when no retry is configured). Earlier attempts inside the
				// same task aren't worth saving — they get superseded by the corrected one.
				if (lastAttempt && this._activeAxiomPlan && this._activeAxiomClassification) {
					this._axiomRuntime.captureFailure({
						traceId,
						taskText: this._activeAxiomUserText,
						taskKind: this._activeAxiomClassification.kind,
						abstraction: this._activeAxiomPlan.abstraction,
						failureType: "ip-validation",
						failureCodes: errorIssues.map((i) => i.code),
						cause: errorIssues.map((i) => `[${i.category}] ${i.message}`).join(" / "),
						correction: errorIssues
							.map((i) => i.fixHint)
		...
		this._axiomRuntime.recordAgentEvent(traceId, event);
	}
	...
	/** Current AXIOM runtime effort/features. */
	get axiomSettings(): ResolvedAxiomSettings {
		return this.settingsManager.getAxiomSettings();
	}
	...
	private _canRetryOrFallback(message: AssistantMessage): boolean {
		const settings = this.settingsManager.getRetrySettings();
		if (!settings.enabled || !this._isRetryableError(message)) {
			return false;
		}
		return this._retryAttempt < settings.maxRetries || this._getRetryFallbackModel() !== undefined;
	}

	private _shouldSuppressRetryErrorEvent(event: AgentEvent): boolean {
		if (event.type === "message_start" || event.type === "message_end") {
			return event.message.role === "assistant" && this._canRetryOrFallback(event.message as AssistantMessage);
		}
	...
	private _createUserMessage(text: string, images?: ImageContent[]): Extract<AgentMessage, { role: "user" }> {
	...
	private _createLocalAssistantMessage(text: string): AssistantMessage {
		return {
			...
			api: "local" as AssistantMessage["api"],
			provider: "axiom-runtime" as AssistantMessage["provider"],
	...
	private _startAxiomTrace(
		input: string,
		classification: AxiomTaskClassification,
		plan?: AxiomTaskPlan,
	): { id: string; startedAt: number } {
		const traceId = classification.id;
		const startedAt = Date.now();
		this._activeAxiomTraceId = traceId;
		this._activeAxiomUserText = input;
		this._activeAxiomClassification = classification;
		this._activeAxiomPlan = plan;
		this._axiomGraphTracker = plan?.graph ? new GraphExecutionTracker(plan.graph) : undefined;
		this._axiomIPRetryCount = 0;
		...
		return { id: traceId, startedAt };
	}

	private _finishAxiomTrace(
		trace: { id: string; startedAt: number } | undefined,
		outcome: "completed" | "error",
		errorMessage?: string,
	): void {
		// Reflexion: capture a failure reflection on task errors so future similar tasks
		// can avoid the same trap. We have an active plan + classification + user text.
		if (
			outcome === "error" &&
			trace &&
			this._activeAxiomPlan &&
			this._activeAxiomClassification &&
			this._activeAxiomUserText
		) {
			this._axiomRuntime.captureFailure({
				traceId: trace.id,
				taskText: this._activeAxiomUserText,
				taskKind: this._activeAxiomClassification.kind,
				abstraction: this._activeAxiomPlan.abstraction,
				failureType: "task-error",
				failureCodes: ["task-error"],
				cause: errorMessage ?? "Task ended with an unrecovered error.",
				correction:
					"Inspect the trace JSONL for the error context. Common causes: tool failure, provider timeout, IP rejection past retry limit.",
			});
		}

		// Skill Evolution: capture successful procedural patterns. The bar to qualify is
		// deliberately high — non-trivial complexity, actual tool use, and no IP retries.
		// Otherwise the skill library fills up with low-signal entries that pollute recall.
		if (
			outcome === "completed" &&
			trace &&
			this._activeAxiomPlan &&
			...
			const axiomSettings = this.settingsManager.getAxiomSettings();
			const complexity = this._activeAxiomClassification.complexity;
			const tools = this._axiomToolsUsedThisTask;
			const qualifies =
				axiomSettings.skillEvolution &&
				this._axiomIPRetryCount === 0 &&
				tools.length > 0 &&
				complexity >= axiomSettings.skillMinComplexity;
			if (qualifies) {
				this._axiomRuntime.captureSkill({
					traceId: trace.id,
					taskText: this._activeAxiomUserText,
					taskKind: this._activeAxiomClassification.kind,
					abstraction: this._activeAxiomPlan.abstraction,
		...
		if (trace && this._axiomGraphTracker) {
			this._axiomRuntime.recordGraphExecutionSummary(trace.id, this._axiomGraphTracker.snapshot());
		}

		// Confidence tracking: credit every recalled skill on completion (provided
		// no IP retries fired — a retried task ran rougher than the skill suggested),
		// debit every recalled skill on error. Skipped when the plan recalled none.
		if (this._activeAxiomPlan && this._activeAxiomPlan.skillRecalls.length > 0) {
			const recalledIds = this._activeAxiomPlan.skillRecalls.map((hit) => hit.skill.id);
			if (outcome === "completed" && this._axiomIPRetryCount === 0) {
				this._axiomRuntime.recordSkillOutcome(trace?.id, "success", recalledIds);
		...
		if (trace?.id === this._activeAxiomTraceId) {
			this._activeAxiomTraceId = undefined;
			this._activeAxiomUserText = "";
			this._activeAxiomClassification = undefined;
			this._activeAxiomPlan = undefined;
			this._axiomGraphTracker = undefined;
			this._axiomIPRetryCount = 0;
			this._axiomToolsUsedThisTask = [];
			this._axiomStreamAbortsThisTask = 0;
			this._axiomStreamAbortPending = false;
	...
	private async _runLocalFastPath(
		userMessage: Extract<AgentMessage, { role: "user" }>,
		classification: AxiomTaskClassification,
	): Promise<void> {
		const reply = classification.fastPathReply;
		if (!reply) {
			return;
		}

		const trace = this._startAxiomTrace(this._getUserMessageText(userMessage), classification);
		const assistantMessage = this._createLocalAssistantMessage(reply);
		const newMessages: AgentMessage[] = [userMessage, assistantMessage];
		try {
			await this._handleAgentEvent({ type: "agent_start" });
			await this._handleAgentEvent({ type: "turn_start" });
			await this._handleAgentEvent({ type: "message_start", message: userMessage });
			this.agent.state.messages.push(userMessage);
			await this._handleAgentEvent({ type: "message_end", message: userMessage });
			await this._handleAgentEvent({ type: "message_start", message: assistantMessage });
			this.agent.state.messages.push(assistantMessage);
			await this._handleAgentEvent({ type: "message_end", message: assistantMessage });
			await this._handleAgentEvent({ type: "turn_end", message: assistantMessage, toolResults: [] });
			await this._handleAgentEvent({ type: "agent_end", messages: newMessages });
			this._axiomRuntime.recordLocalFastPath(trace.id, reply, Date.now() - trace.startedAt);
			this._finishAxiomTrace(trace, "completed");
		} catch (error) {
			this._finishAxiomTrace(trace, "error");
			throw error;
		}
	}
	...
	private async _runAgentPrompt(messages: AgentMessage | AgentMessage[]): Promise<void> {
		try {
			this._restorePrimaryModelAfterFallback();
	...
	async prompt(text: string, options?: PromptOptions): Promise<void> {
		...
		let axiomClassification: AxiomTaskClassification | undefined;
		let axiomPlan: AxiomTaskPlan | undefined;
		let expandedPromptText = "";
		// ASCoT-driven per-turn thinking level override. We snapshot the user's level
		// before the turn and restore it in the execution `finally`, so the override
		// never persists into the next prompt or into saved settings.
		let previousAxiomThinkingLevel: ThinkingLevel | undefined;
		...
		try {
			...
			expandedPromptText = expandedText;
			...
			axiomClassification = this._axiomRuntime.classifyPrompt({
				text: expandedText,
				hasImages: Boolean(currentImages?.length),
				hasPendingContext: this._pendingNextTurnMessages.length > 0,
				activeToolNames: this.getActiveToolNames(),
				model: this.model,
				thinkingLevel: this.thinkingLevel,
			});

			if (axiomClassification.route === "local" && axiomClassification.fastPathReply) {
				preflightResult?.(true);
				await this._runLocalFastPath(this._createUserMessage(expandedText, currentImages), axiomClassification);
				return;
			}
			...
			// AXIOM: per-task plan (Step-Back abstraction + Reflexion recall + ASCoT depth).
			// Best-effort: failure to plan never blocks the task. Skipped entirely when
			// AXIOM is disabled — planTask() short-circuits internally.
			try {
				let auth: { apiKey: string; headers?: Record<string, string> } | undefined;
				try {
					auth = await this._getRequiredRequestAuth(this.model);
				} catch {
					// Auth was checked above; if it fails here (e.g., race), planTask
					// will fall back to the keyword-only abstraction.
				}
				axiomPlan = await this._axiomRuntime.planTask({
					traceId: axiomClassification.id,
					text: expandedText,
				...
				// Planning is best-effort; the rest of the flow proceeds without it.
				axiomPlan = undefined;
			}
			...
			messages = [this._createUserMessage(expandedText, currentImages)];
			...
			if (axiomPlan) {
				const axiomAppend = this._axiomRuntime.buildSystemPromptAppend(axiomPlan);
				if (axiomAppend) {
					this.agent.state.systemPrompt = `${this.agent.state.systemPrompt}\n\n${axiomAppend}`;
				}

				// ASCoT depth modulation: push the planner's chosen thinking level into
				// the agent state for this turn only. Skipped when ASCoT is disabled,
				// when the model doesn't support thinking, or when the level is already
				// what we want. The "off" case is honored even on thinking-capable models
				// (e.g., greetings) so trivial replies don't burn reasoning tokens.
				const axiomSettings = this.settingsManager.getAxiomSettings();
				if (axiomSettings.ascotDepth) {
					const target = axiomPlan.ascot.thinkingLevel as ThinkingLevel;
					const available = this.getAvailableThinkingLevels();
					const effective: ThinkingLevel = available.includes(target)
						? target
						: this._clampThinkingLevel(target, available);
					if (effective !== this.agent.state.thinkingLevel) {
						previousAxiomThinkingLevel = this.agent.state.thinkingLevel;
		...
		preflightResult?.(true);
		const trace = axiomClassification
			? this._startAxiomTrace(expandedPromptText, axiomClassification, axiomPlan)
			: undefined;
		try {
			...
			this._finishAxiomTrace(trace, "error", message);
			throw error;
		} finally {
			if (previousAxiomThinkingLevel !== undefined) {
				this.agent.state.thinkingLevel = previousAxiomThinkingLevel;
			}
		}
	}
	...
	setAxiomEffort(effort: AxiomEffortLevel): ResolvedAxiomSettings {
		this.settingsManager.setAxiomEffort(effort);
	...
	cycleAxiomEffort(): ResolvedAxiomSettings {
		const settings = this.settingsManager.cycleAxiomEffort();
		this._refreshAxiomRuntime();
		this._emit({ type: "axiom_effort_changed", settings });
		return settings;
	}
	...
	private _primaryProvider(): string {
		return process.env.AXIOM_PRIMARY_PROVIDER || "google";
	}
	...
	private _fallbackProvider(): string {
		return process.env.AXIOM_FALLBACK_PROVIDER || "nvidia-nim";
	}
	...
	private _isPrimaryModel(model: Model<any> | undefined): boolean {
		return model?.provider === this._primaryProvider() && model.id === this._primaryModelId();
	}

	private _isFallbackModel(model: Model<any> | undefined): boolean {
		return model?.provider === this._fallbackProvider() && model.id === this._fallbackModelId();
	}

	private _getRetryFallbackModel(): Model<any> | undefined {
		if (this._fallbackRetryActive || !this._isPrimaryModel(this.model)) {
			return undefined;
		}
		const fallback = this._modelRegistry.find(this._fallbackProvider(), this._fallbackModelId());
		if (!fallback || !this._modelRegistry.hasConfiguredAuth(fallback)) {
			return undefined;
		}
		return fallback;
	}

	private _restorePrimaryModelAfterFallback(): void {
		if (!this._fallbackRetryActive || !this._isFallbackModel(this.model)) {
			return;
		}
		const primary = this._modelRegistry.find(this._primaryProvider(), this._primaryModelId());
		if (!primary || !this._modelRegistry.hasConfiguredAuth(primary)) {
			return;
		}
		this.agent.state.model = primary;
		this._fallbackRetryActive = false;
	}

	private async _switchToRetryFallback(message: AssistantMessage): Promise<boolean> {
		const fallback = this._getRetryFallbackModel();
		if (!fallback) {
			return false;
		}

		const previousModel = this.model;
		this._fallbackRetryActive = true;
		this._retryAttempt = 0;
		this.agent.state.model = fallback;
		this.sessionManager.appendModelChange(fallback.provider, fallback.id);
		await this._emitModelSelect(fallback, previousModel, "restore");
		...
		const delayMs = this.settingsManager.getRetrySettings().baseDelayMs;
		this._emit({
			...
			delayMs,
			errorMessage:
				`Primary model failed after retries; switching to NVIDIA NIM fallback. ${message.errorMessage || ""}`.trim(),
		});
		...
		try {
			...
			this._emit({
				...
				success: false,
				attempt: 0,
				finalError: "Fallback retry cancelled",
			});
			return false;
		} finally {
	...
	private async _prepareRetry(message: AssistantMessage): Promise<boolean> {
		...
		if (this._retryAttempt > settings.maxRetries) {
			...
			this._retryAttempt--;
			if (await this._switchToRetryFallback(message)) {
				return true;
			}
			return false;
		}
