 ...
 * - <cwd>/.axiom/presets.json (project-local)
 *
...
import type { Api, Model } from "@axiom/ai";
import type { ExtensionAPI, ExtensionContext } from "@axiom/coding-agent";
import { DynamicBorder, getAgentDir } from "@axiom/coding-agent";
import { Container, Key, type SelectItem, SelectList, Text } from "@axiom/tui";
...
function loadPresets(cwd: string): PresetsConfig {
	...
	const projectPath = join(cwd, ".axiom", "presets.json");
...
export default function presetExtension(pi: ExtensionAPI) {
	...
	async function showPresetSelector(ctx: ExtensionContext): Promise<void> {
		...
		if (presetNames.length === 0) {
			ctx.ui.notify(
				"No presets defined. Add presets to ~/.axiom/agent/presets.json or .axiom/presets.json",
				"warning",
			);
			return;
		}
