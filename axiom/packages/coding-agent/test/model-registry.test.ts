...
import type { AnthropicMessagesCompat, Api, Context, Model, OpenAICompletionsCompat } from "@axiom/ai";
import { getApiProvider } from "@axiom/ai";
import { getOAuthProvider } from "@axiom/ai/oauth";
...
describe("ModelRegistry", () => {
	...
	beforeEach(() => {
		tempDir = join(tmpdir(), `axiom-test-model-registry-${Date.now()}-${Math.random().toString(36).slice(2)}`);
