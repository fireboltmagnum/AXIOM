...
import { getModel, type OAuthCredentials, type OAuthProvider } from "@axiom/ai";
...
// ============================================================================
// OAuth API key resolution from ~/.axiom/agent/auth.json
// ============================================================================

const AUTH_PATH = join(homedir(), ".axiom", "agent", "auth.json");
...
/**
 * Resolve API key for a provider from ~/.axiom/agent/auth.json
 *
...
/**
 * Check if a provider has credentials in ~/.axiom/agent/auth.json
 */
...
export const PI_AGENT_DIR = join(homedir(), ".axiom", "agent");

/**
 * Get an AuthStorage instance backed by ~/.axiom/agent/auth.json
...
export function createTestSession(options: TestSessionOptions = {}): TestSessionContext {
	const tempDir = join(tmpdir(), `axiom-test-${Date.now()}-${Math.random().toString(36).slice(2)}`);
