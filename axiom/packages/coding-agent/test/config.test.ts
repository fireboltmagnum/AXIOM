...
const originalPiPackageDir = process.env.AXIOM_PACKAGE_DIR;
...
afterEach(() => {
	...
	if (originalPiPackageDir === undefined) {
		delete process.env.AXIOM_PACKAGE_DIR;
	} else {
		process.env.AXIOM_PACKAGE_DIR = originalPiPackageDir;
	}
...
function createNpmPrefixInstall(template = "pi-prefix-"): { prefix: string; packageDir: string } {
	...
	tempDir = prefix;
	process.env.AXIOM_PACKAGE_DIR = packageDir;
...
describe("detectInstallMethod", () => {
	test("detects pnpm from Windows .pnpm install paths", () => {
		...
		expect(getUpdateInstruction("@axiom/coding-agent")).toBe(
			"Run: pnpm install -g --ignore-scripts @axiom/coding-agent",
		);
	});
	...
	test("does not self-update unknown wrapper installs", () => {
		...
		expect(getSelfUpdateCommand("@axiom/coding-agent")).toBeUndefined();
		expect(getUpdateInstruction("@axiom/coding-agent")).toBe(
			"Update @axiom/coding-agent using the package manager, wrapper, or source checkout that provides this installation.",
		);
	});
	...
	test("self-updates npm installs from custom prefixes", () => {
		...
		const command = getSelfUpdateCommand("@axiom/coding-agent");

		expect(detectInstallMethod()).toBe("npm");
		expect(command).toEqual({
			command: "npm",
			args: ["--prefix", prefix, "install", "-g", "--ignore-scripts", "@axiom/coding-agent"],
			display: `npm --prefix ${prefix} install -g --ignore-scripts @axiom/coding-agent`,
		});
	});
	...
	test("self-update respects configured npmCommand", () => {
		...
		const command = getSelfUpdateCommand("@axiom/coding-agent", ["npm", "--prefix", prefix]);
	...
	test("self-update treats empty npmCommand as unset", () => {
		...
		expect(command?.args).toEqual(["--prefix", prefix, "install", "-g", "--ignore-scripts", "@axiom/coding-agent"]);
	});
	...
	test("quotes npm self-update display paths", () => {
		...
		expect(command?.display).toBe(`npm --prefix "${prefix}" install -g --ignore-scripts @axiom/coding-agent`);
	});
	...
	test("does not infer Windows npm custom prefixes from package paths", () => {
		...
		expect(detectInstallMethod()).toBe("npm");
		expect(getUpdateInstruction("@axiom/coding-agent")).toBe(
			"Run: npm install -g --ignore-scripts @axiom/coding-agent",
		);
	});
	...
	test("self-updates bun global installs from bun pm bin", () => {
		...
		expect(command).toEqual({
			command: "bun",
			args: ["install", "-g", "--ignore-scripts", "@axiom/coding-agent"],
			display: "bun install -g --ignore-scripts @axiom/coding-agent",
		});
	});
	...
	test("self-updates pnpm v11 global installs resolved through the store", () => {
		...
		const packageName = "@axiom/coding-agent";
		...
		tempDir = temp;
		process.env.PATH = `${binDir}${delimiter}${originalPath ?? ""}`;
		process.env.AXIOM_PACKAGE_DIR = storePackageDir;
	...
	test("does not self-update when npm install path is not writable", () => {
		...
		expect(getSelfUpdateCommand("@axiom/coding-agent")).toBeUndefined();
		expect(getSelfUpdateUnavailableInstruction("@axiom/coding-agent")).toContain("the install path is not writable");
	});
});
