...
describe("DefaultPackageManager", () => {
	...
	beforeEach(() => {
		previousOfflineEnv = process.env.AXIOM_OFFLINE;
		delete process.env.AXIOM_OFFLINE;
	...
	afterEach(() => {
		if (previousOfflineEnv === undefined) {
			...
			process.env.AXIOM_OFFLINE = previousOfflineEnv;
		}
		vi.restoreAllMocks();
		vi.unstubAllGlobals();
...
Content`,
		...
		it("should auto-discover root markdown skills from .axiom skill dirs", async () => {
...
Content`,
		...
		it("should resolve project paths relative to .axiom", async () => {
			const extDir = join(tempDir, ".axiom", "extensions");
			mkdirSync(extDir, { recursive: true });
...
Content`,
				...
				mkdirSync(join(tempDir, ".axiom"), { recursive: true });
				...
				symlinkSync(sharedExtensionsDir, join(tempDir, ".axiom", "extensions"), "dir");
				symlinkSync(sharedSkillsDir, join(tempDir, ".axiom", "skills"), "dir");
				symlinkSync(sharedPromptsDir, join(tempDir, ".axiom", "prompts"), "dir");
				symlinkSync(sharedThemesDir, join(tempDir, ".axiom", "themes"), "dir");
		...
		it("should auto-discover project prompts with overrides", async () => {
			const promptsDir = join(tempDir, ".axiom", "prompts");
			mkdirSync(promptsDir, { recursive: true });
	...
	describe("auto-discovered skill metadata", () => {
		it("should use the agent dir as baseDir for user .axiom/agent skills", async () => {
		...
		it("should use the project .axiom dir as baseDir for project .axiom skills", async () => {
			const projectBaseDir = join(tempDir, ".axiom");
	...
	describe(".agents/skills auto-discovery", () => {
		...
		it("should dedupe user skill entries when ~/.axiom/agent/skills is a symlink to ~/.agents/skills", async () => {
			const previousHome = process.env.HOME;
			process.env.HOME = tempDir;
	...
	describe("ignore files", () => {
		...
		it("should not apply parent .gitignore to .axiom auto-discovery", async () => {
			writeFileSync(join(tempDir, ".gitignore"), ".axiom\n");

			const skillDir = join(tempDir, ".axiom", "skills", "auto-skill");
			mkdirSync(skillDir, { recursive: true });
	...
	describe("npmCommand", () => {
		...
		it("should update git package dependencies with --omit=dev", async () => {
			const source = "git:github.com/user/repo";
			const targetDir = join(tempDir, ".axiom", "git", "github.com", "user", "repo");
			mkdirSync(targetDir, { recursive: true });
			writeFileSync(join(targetDir, "package.json"), JSON.stringify({ name: "repo", version: "1.0.0" }));
	...
	describe("settings source normalization", () => {
		...
		it("should store project local packages relative to .axiom settings base", () => {
			...
			const rel = relative(join(tempDir, ".axiom"), projectPkgDir);
			const expected = rel.startsWith(".") ? rel : `./${rel}`;
			expect(settings.packages?.[0]).toBe(expected);
		});
...
export default function(api) { api.registerTool({ name: "test", description: "test", execute: async () => helper() }); }`,
	...
	describe("offline mode and network timeouts", () => {
		it("should update project npm packages using @latest when newer version is available", async () => {
			const installedPath = join(tempDir, ".axiom", "npm", "node_modules", "example");
			...
			expect(runCommandSpy).toHaveBeenCalledWith(
				"npm",
				["install", "example@latest", "--prefix", join(tempDir, ".axiom", "npm")],
				undefined,
			);
		});
		...
		it("should batch npm updates per scope and run git updates in parallel while skipping pinned npm and current packages", async () => {
			...
			const projectOldPath = join(tempDir, ".axiom", "npm", "node_modules", "project-old");
			const projectCurrentPath = join(tempDir, ".axiom", "npm", "node_modules", "project-current");
			...
			expect(runCommandSpy).toHaveBeenNthCalledWith(
				2,
				"npm",
				["install", "project-old@latest", "project-missing@latest", "--prefix", join(tempDir, ".axiom", "npm")],
				undefined,
			);
		...
		it("should skip pinned packages when checking for updates", async () => {
			const installedNpmPath = join(tempDir, ".axiom", "npm", "node_modules", "example");
