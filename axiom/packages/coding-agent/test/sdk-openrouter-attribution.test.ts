...
describe("createAgentSession OpenRouter attribution headers", () => {
	...
	beforeEach(() => {
		...
		originalTelemetryEnv = process.env.AXIOM_TELEMETRY;
		delete process.env.AXIOM_TELEMETRY;
	});
	...
	afterEach(() => {
		if (originalTelemetryEnv === undefined) {
			...
			process.env.AXIOM_TELEMETRY = originalTelemetryEnv;
		}
	...
	it("adds default attribution headers for OpenRouter models", async () => {
		...
		// AXIOM does not ship an HTTP-Referer; only the title and category survive.
		expect(headers?.["HTTP-Referer"]).toBeUndefined();
		expect(headers?.["X-OpenRouter-Title"]).toBe("axiom");
	...
	it("adds OpenCode session headers", async () => {
		...
		expect(headers?.["x-opencode-client"]).toBe("axiom");
	});
