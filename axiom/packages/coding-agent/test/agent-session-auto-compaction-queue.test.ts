...
import { type AssistantMessage, getModel } from "@axiom/ai";
...
vi.mock("../src/core/compaction/index.js", () => ({
	...
	shouldCompact: (
		...
		settings: { enabled: boolean; reserveTokens: number; thresholdRatio?: number },
	) => {
		const usableContext = Math.max(1, contextWindow - Math.max(0, settings.reserveTokens));
		const ratio = settings.thresholdRatio ?? 0.9;
		return settings.enabled && contextTokens > Math.floor(usableContext * ratio);
	},
}));
