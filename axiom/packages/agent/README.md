...
Stateful agent with tool execution and event streaming. Built on `@axiom/ai`.

## Installation

```bash
npm install @axiom/agent-core
```
...
```typescript
declare module "@axiom/agent-core" {
...
```typescript
import { Agent, streamProxy } from "@axiom/agent-core";
...
```typescript
import { agentLoop, agentLoopContinue } from "@axiom/agent-core";
